...
TCPTestHarness TCPTestHarness::in_last_ack(const TCPConfig &cfg,
    ...
    h.execute(Close{});
    h.execute(ExpectOneSegment{}.with_no_flags().with_fin(true).with_ack(true).with_seqno(tx_isn + 1).with_ackno(rx_isn + 2));
    return h;
}
...
TCPTestHarness TCPTestHarness::in_fin_wait_1(const TCPConfig &cfg,
    ...
    TCPTestHarness h = in_established(cfg, tx_isn, rx_isn);
    h.execute(Close{});
    h.execute(ExpectOneSegment{}.with_no_flags().with_fin(true).with_ack(true).with_ackno(rx_isn + 1).with_seqno(tx_isn + 1));
    return h;
}
